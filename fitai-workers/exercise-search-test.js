...
async function signInAndTest(email, password) {
	...
	const basicResponse = await fetch(`${WORKER_URL}/exercises/search?limit=10`, {
		headers: {
	...
	const queryResponse = await fetch(`${WORKER_URL}/exercises/search?query=chest&limit=5`, {
		headers: {
			Authorization: `Bearer ${token}`,
		},
	});
	...
	const equipmentResponse = await fetch(`${WORKER_URL}/exercises/search?equipment=dumbbell&limit=5`, {
		headers: {
			Authorization: `Bearer ${token}`,
		},
	});
	...
	if (equipmentResponse.ok) {
		...
		equipmentData.data.exercises.forEach((ex, idx) => {
			console.log(`${idx + 1}. ${ex.name} (${ex.equipment}) - ${ex.bodyPart}`);
		});
	} else {
	...
	const bodyPartResponse = await fetch(`${WORKER_URL}/exercises/search?bodyParts=chest&limit=5`, {
		headers: {
			Authorization: `Bearer ${token}`,
		},
	});
	...
	if (bodyPartResponse.ok) {
		...
		bodyPartData.data.exercises.forEach((ex, idx) => {
			console.log(`${idx + 1}. ${ex.name} (${ex.equipment}) - ${ex.target}`);
		});
	} else {
	...
	const muscleResponse = await fetch(`${WORKER_URL}/exercises/search?muscles=pectorals&limit=5`, {
		headers: {
			Authorization: `Bearer ${token}`,
		},
	});
	...
	if (muscleResponse.ok) {
		...
		muscleData.data.exercises.forEach((ex, idx) => {
			console.log(`${idx + 1}. ${ex.name} (${ex.equipment}) - Target: ${ex.target}`);
		});
	} else {
	...
	const combinedResponse = await fetch(`${WORKER_URL}/exercises/search?bodyParts=chest&equipment=barbell&limit=5`, {
		headers: {
			Authorization: `Bearer ${token}`,
		},
	});
	...
	const paginationResponse = await fetch(`${WORKER_URL}/exercises/search?offset=10&limit=5`, {
		headers: {
			Authorization: `Bearer ${token}`,
		},
	});
