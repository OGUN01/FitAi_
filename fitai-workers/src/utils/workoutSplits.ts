...
const FULL_BODY_SPLIT: WorkoutSplit = {
	...
	description:
		'Train all major muscle groups 3 times per week. Ideal for beginners and those with limited time. Maximum efficiency and recovery.',
	idealFrequency: [3, 3],
	daysPerWeek: 3,
	workoutDays: [
		{
...
const UPPER_LOWER_SPLIT: WorkoutSplit = {
	...
	description:
		'Alternate between upper and lower body days. Great balance of volume, frequency, and recovery. Suitable for most intermediate lifters.',
	idealFrequency: [4, 4],
	daysPerWeek: 4,
	workoutDays: [
		{
...
const PPL_3X_SPLIT: WorkoutSplit = {
	...
	description:
		'Classic PPL split. Push day (chest, shoulders, triceps), Pull day (back, biceps), Legs day (quads, hamstrings, glutes). Once per week frequency.',
	idealFrequency: [3, 3],
	daysPerWeek: 3,
	workoutDays: [
		{
...
const PPL_6X_SPLIT: WorkoutSplit = {
	...
	description:
		'High-frequency PPL. Train each muscle group twice per week. For advanced lifters with good recovery capacity. Maximum muscle growth potential.',
	idealFrequency: [6, 6],
	daysPerWeek: 6,
	workoutDays: [
		{
...
const BRO_SPLIT: WorkoutSplit = {
	...
	description:
		'Classic bodybuilding split. One major muscle group per day. High volume per muscle, long recovery between sessions. Focus on isolation and pump.',
	idealFrequency: [6, 6],
	daysPerWeek: 5,
	workoutDays: [
		{
...
const UL_PPL_HYBRID_SPLIT: WorkoutSplit = {
	id: 'ul_ppl_hybrid_5x',
	name: 'Upper/Lower + PPL Hybrid 5x/Week',
	description:
		'Hybrid 5-day split combining Upper/Lower with Push/Pull/Legs. Each muscle group hit 2-3x per week. High frequency, moderate-high volume. Ideal for intermediate to advanced lifters.',
	idealFrequency: [5, 5],
	daysPerWeek: 5,
	workoutDays: [
		{
...
const HIIT_CIRCUIT_SPLIT: WorkoutSplit = {
	...
	description:
		'High-intensity interval training and circuit workouts. Combines strength and cardio. Maximum calorie burn, improved conditioning. Short, intense sessions.',
	idealFrequency: [3, 4],
	daysPerWeek: 4,
	workoutDays: [
		{
...
const FULL_BODY_2X_SPLIT: WorkoutSplit = {
	id: 'full_body_2x',
	name: 'Full Body 2x/Week',
	description:
		'Two full-body workouts per week with A/B alternation. Perfect for beginners or those with very limited schedules. Covers all major muscle groups each session.',
	idealFrequency: [2, 2],
	daysPerWeek: 2,
	workoutDays: [
		{
	...
	fitnessGoals: ['general_fitness', 'weight_loss', 'muscle_gain', 'strength', 'maintenance', 'flexibility'],
	minimumEquipment: ['body weight', 'dumbbell'],
	volumePerMuscle: 'low',
	recoveryDemand: 'low',
	timePerSession: 40,
...
const ACTIVE_RECOVERY_SPLIT: WorkoutSplit = {
	...
	description:
		'Low-intensity, recovery-focused workouts. Mobility, flexibility, light resistance. Ideal for high stress, seniors, or those prioritizing recovery.',
	idealFrequency: [2, 3],
	daysPerWeek: 2,
	workoutDays: [
		{
...
function scoreSplit(split: WorkoutSplit, profile: UserProfile): { score: number; breakdown: string[] } {
	...
	// Beginner hard blocks: absolute guards, not score reductions
	if (profile.experienceLevel === 'beginner') {
		if (split.id === 'ppl_6x' || split.id === 'bro_split_5x') {
			return { score: 0, breakdown: ['✗ Blocked: split not suitable for beginners'] };
		}
		if (split.recoveryDemand === 'high') {
			return { score: 0, breakdown: ['✗ Blocked: high recovery demand not suitable for beginners'] };
		}
	}
	...
	const prefersVariety = profile.prefersVariety ?? false;
