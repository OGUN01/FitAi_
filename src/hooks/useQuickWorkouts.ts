...
export const useQuickWorkouts = (navigation: FitnessNavigation): QuickWorkoutsHook => {
  ...
  const isVisible = useMemo(() => {
    ...
    // Audit #6 — rest days are the best cross-sell moment for a light
    // yoga/walk session. A day with no planned workout IS a rest day, so show
    // quick workouts instead of leaving the day a dead end.
    if (!hasTodayPlannedWorkout) return true;
