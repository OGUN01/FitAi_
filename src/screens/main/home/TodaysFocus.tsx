...
export const TodaysFocus: React.FC<TodaysFocusProps> = ({
  ...
  const getWorkoutSubtitle = () => {
    if (!workoutInfo.hasWeeklyPlan) return 'Generate a personalized workout plan';
    ...
    if (workoutInfo.hasWorkout && workoutInfo.workout) {
      const parts = [];
      if (workoutInfo.workout.duration) parts.push(`${workoutInfo.workout.duration} min`);
      if (workoutInfo.workout.exercises) parts.push(`${workoutInfo.workout.exercises} exercises`);
      if (workoutInfo.workout.estimatedCalories) parts.push(`~${workoutInfo.workout.estimatedCalories} cal`);
      ...
      <GlassCard elevation={2} blurIntensity="light" padding="md" borderRadius="lg">
        <View style={styles.container}>
          ...
          <View style={styles.textContainer}>
            <Text style={styles.title} numberOfLines={1}>{getWorkoutTitle()}</Text>
            <Text style={styles.subtitle} numberOfLines={1}>{getWorkoutSubtitle()}</Text>
            
            {/* Progress bar (only if in progress) */}
            {workoutProgress > 0 && workoutProgress < 100 && !workoutInfo.isRestDay && (
              <View style={styles.progressContainer}>
                <View style={styles.progressBar}>
                  <View style={[styles.progressFill, { width: `${workoutProgress}%`, backgroundColor: color }]} />
                </View>
                <Text style={[styles.progressText, { color }]}>{workoutProgress}%</Text>
              </View>
            )}
          </View>
