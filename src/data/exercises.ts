...
export const EXERCISES: Exercise[] = [
  ...
  },
  {
    ...
    restTime: 60,
    calories: 10,
    tips: ['Keep hips level', 'Maintain plank position', 'Drive knees toward chest'],
    variations: [
  ...
  // FLEXIBILITY/YOGA EXERCISES
  {
    ...
    description:
      'Yoga pose that stretches hamstrings, calves, and shoulders while strengthening arms',
    instructions: [
  ...
  },
  {
    ...
    restTime: 0,
    calories: 1,
    tips: ['Breathe deeply and relax', 'Widen knees if more comfortable', 'Use as a rest position'],
    variations: ["Extended Child's Pose", "Side Child's Pose", 'Thread the Needle'],
  },
  {
    id: 'sun_salutation',
    name: 'Sun Salutation',
    description: 'Classic yoga flow that stretches and strengthens the entire body through a sequence of poses',
    instructions: [
      'Start in mountain pose with feet together',
      'Inhale, raise arms overhead',
      'Exhale, fold forward touching the floor',
      'Step or jump back to plank, lower to floor',
    ...
    tips: [
      ...
      'Modify poses as needed for your flexibility level',
    ],
    variations: ['Half Sun Salutation', 'Sun Salutation B', 'Modified Sun Salutation'],
  },
  {
    id: 'warrior_pose',
    name: 'Warrior Pose',
    description: 'Standing yoga pose that builds leg strength, balance, and hip flexibility',
    instructions: [
      'Step one foot back about 3-4 feet',
      'Bend front knee to 90 degrees',
      'Keep back leg straight with foot angled out',
      'Raise arms overhead (Warrior I) or extend to sides (Warrior II)',
    ...
    tips: [
      ...
      'Press through the outer edge of back foot',
      'Engage core for balance',
    ],
    variations: ['Warrior I', 'Warrior II', 'Warrior III', 'Reverse Warrior'],
  },
  ...
  // HIIT EXERCISES
  {
    ...
    name: 'High Knees',
    description:
      'High-intensity cardio exercise that improves coordination and elevates heart rate',
    instructions: [
    ...
    restTime: 30,
    calories: 12,
    tips: ['Stay on balls of feet', 'Keep core engaged', 'Drive knees up high'],
  ...
  },
  {
    ...
    restTime: 60,
    calories: 15,
    tips: ['Keep core tight', 'Use whole body, not just arms', 'Maintain consistent wave pattern'],
...
export const MUSCLE_GROUPS = {
  ...
  BACK: {
    name: 'Back',
    exercises: EXERCISES.filter(
      (ex) => ex.muscleGroups.includes('back') || ex.muscleGroups.includes('lats')
    ),
  },
  SHOULDERS: {
    name: 'Shoulders',
    exercises: EXERCISES.filter((ex) => ex.muscleGroups.includes('shoulders')),
  },
  ARMS: {
    name: 'Arms',
    exercises: EXERCISES.filter(
      (ex) => ex.muscleGroups.includes('biceps') || ex.muscleGroups.includes('triceps')
    ),
  },
  LEGS: {
    name: 'Legs',
    exercises: EXERCISES.filter(
      (ex) =>
        ex.muscleGroups.includes('quadriceps') ||
        ex.muscleGroups.includes('hamstrings') ||
        ex.muscleGroups.includes('calves')
    ),
  },
  GLUTES: {
    name: 'Glutes',
    exercises: EXERCISES.filter((ex) => ex.muscleGroups.includes('glutes')),
  },
  CORE: { name: 'Core', exercises: EXERCISES.filter((ex) => ex.muscleGroups.includes('core')) },
  FULL_BODY: {
    name: 'Full Body',
    exercises: EXERCISES.filter((ex) => ex.muscleGroups.includes('full_body')),
  },
};
...
export const EQUIPMENT_CATEGORIES = {
  BODYWEIGHT: {
    ...
    exercises: EXERCISES.filter(
      (ex) => ex.equipment.includes('bodyweight') && ex.equipment.length === 1
    ),
  },
  DUMBBELLS: {
    name: 'Dumbbells',
...
export const getExercisesByEquipment = (equipment: string[]): Exercise[] => {
  return EXERCISES.filter((exercise) => equipment.some((eq) => exercise.equipment.includes(eq)));
};
