...
export const REGIONAL_CUISINE_DATA = {
  north: {
    ...
    signature_dishes: [
      'butter chicken', 'dal makhani', 'biryani', 'naan', 'tandoori chicken',
      'rajma', 'chole', 'paratha', 'kulcha', 'sarson ka saag'
    ],
  ...
  south: {
    ...
    signature_dishes: [
      'dosa', 'idli', 'sambar', 'rasam', 'fish curry', 'vada', 'uttapam',
      'coconut chutney', 'avial', 'pongal', 'chettinad chicken'
    ],
  ...
  east: {
    ...
    characteristics: {
      ...
      calorieModifier: 1.00, // Baseline calories
      fatModifier: 1.00
    },
    signature_dishes: [
      'fish curry', 'prawn malai curry', 'mishti doi', 'rosogolla', 'kathi roll',
      'aloo posto', 'shukto', 'chingri machher jhol'
    ],
  ...
  west: {
    ...
    signature_dishes: [
      'dhokla', 'thepla', 'pav bhaji', 'vada pav', 'undhiyu', 'fish curry',
      'puran poli', 'misal pav', 'bhel puri'
    ],
    flavor_profile: 'sweet-savory, tangy, street food style, medium spice'
  },

  northeast: {
    states: ['assam', 'manipur', 'nagaland', 'mizoram', 'arunachal pradesh', 'tripura', 'meghalaya', 'sikkim'],
    characteristics: {
    ...
    signature_dishes: [
      'fish tenga', 'bamboo shoot curry', 'thukpa', 'momos', 'pitha',
      'khar', 'masor tenga'
    ],
...
export const getRegionalCharacteristics = (region: keyof typeof REGIONAL_CUISINE_DATA) => {
  return REGIONAL_CUISINE_DATA[region]?.characteristics || REGIONAL_CUISINE_DATA.north.characteristics;
};

export const isRegionalDish = (dishName: string, region: keyof typeof REGIONAL_CUISINE_DATA): boolean => {
  const regionData = REGIONAL_CUISINE_DATA[region];
  if (!regionData) return false;
  
  const lowerDishName = dishName.toLowerCase();
  return regionData.signature_dishes.some(dish => 
    lowerDishName.includes(dish.toLowerCase()) || dish.toLowerCase().includes(lowerDishName)
  );
};

export const detectRegionFromDish = (dishName: string): keyof typeof REGIONAL_CUISINE_DATA | null => {
  const lowerDishName = dishName.toLowerCase();
  
  for (const [region, data] of Object.entries(REGIONAL_CUISINE_DATA)) {
    if (data.signature_dishes.some(dish => 
      lowerDishName.includes(dish.toLowerCase()) || dish.toLowerCase().includes(lowerDishName)
    )) {
