...
export interface ValidationResults {
  ...
  calculatedMetrics: {
    ...
    weeklyRate: number;
    originalWeeklyRate: number; // User's originally requested rate (before any capping)
    wasRateCapped: boolean; // True if the rate was adjusted for safety
    protein: number;
    carbs: number;
    fat: number;
    timeline: number;
...
export class ValidationEngine {
  ...
  static validateUserPlan(
    ...
    // Apply age modifier
    tdee = MetabolicCalculations.applyAgeModifier(
      tdee,
      personalInfo.age,
      personalInfo.gender,
    );
    ...
    if (isWeightLoss) {
      ...
      const minCheck = this.validateAbsoluteMinimum(
        targetCalories,
        personalInfo.gender,
      );
    ...
    const goalCheck = this.validateGoalConflict(
    ...
    if (errors.length === 0) {
      ...
      if (isWeightLoss || isWeightGain) {
        const timelineWarn = this.warnAggressiveTimeline(
          requiredWeeklyRate,
          bodyAnalysis.current_weight_kg,
          bodyAnalysis.target_weight_kg,
          bodyAnalysis.target_timeline_weeks,
      ...
      const teenWarn = this.warnTeenAthlete(
        personalInfo.age,
      ...
      const heartWarn = this.warnHeartDisease(
        bodyAnalysis.medical_conditions,
        workoutPreferences.intensity,
      );
      ...
      const interferenceWarn = this.warnConcurrentTrainingInterference(
        workoutPreferences.primary_goals,
      );
      ...
      const obesityWarn = this.warnObesitySpecialGuidance(
        bmi,
        requiredWeeklyRate,
        bodyAnalysis.current_weight_kg,
      );
      ...
      const zeroExerciseWarn = this.warnZeroExercise(
        workoutPreferences.workout_frequency_per_week,
        isWeightLoss ? "weight-loss" : "other",
      );
      ...
      const menopauseWarn = this.warnMenopause(
        personalInfo.gender,
        personalInfo.age,
      );
      ...
      if (isWeightGain) {
        const gainWarn = this.warnExcessiveWeightGain(
          requiredWeeklyRate,
          bodyAnalysis.current_weight_kg,
        );
    ...
    // Determine if rate was capped (only for weight loss scenarios)
    const wasRateCapped =
    ...
    return {
      ...
      calculatedMetrics: {
        ...
        originalWeeklyRate: Math.round(requiredWeeklyRate * 100) / 100, // User's original requested rate
        wasRateCapped, // True if deficit was limited and rate was recalculated
  ...
  private static validateBMRSafety(
    targetCalories: number,
    bmr: number,
  ): ValidationResult {
  ...
  private static validateAbsoluteMinimum(
    targetCalories: number,
    gender: string,
  ): ValidationResult {
  ...
  private static validateGoalConflict(
    primaryGoals: string[],
  ): ValidationResult {
    ...
    if (hasWeightLoss && hasWeightGain) {
      return {
        ...
        recommendations: [
          "Choose your primary goal: weight loss OR weight gain",
        ],
      };
    }
  ...
  private static warnAggressiveTimeline(
    ...
    const extremeLimit = currentWeight * 0.015; // 1.5% - BLOCKED above this
    const aggressiveMax = currentWeight * 0.0125; // 1.25% - Very aggressive
    const moderateAggressive = currentWeight * 0.01; // 1% - Moderately aggressive
    const optimal = currentWeight * 0.0075; // 0.75% - Optimal for muscle retention
    const conservative = currentWeight * 0.005; // 0.5% - Conservative
    const weightDifference = Math.abs(currentWeight - targetWeight);

    // Show warning and alternatives for any rate above optimal (0.75%) and below extreme (1.5%)
    // This captures: 0.75% < rate <= 1.5%
    if (requiredRate > optimal && requiredRate <= extremeLimit) {
      // Calculate alternative timelines for different rates
      const optimalWeeks = Math.ceil(weightDifference / optimal);
      const conservativeWeeks = Math.ceil(weightDifference / conservative);

      // Calculate daily calories for each option
      ...
      // Determine warning severity based on rate
      const isVeryAggressive = requiredRate > moderateAggressive; // > 1%
      const warningMessage = isVeryAggressive
  ...
  private static warnZeroExercise(
    frequency: number,
    goalType: string,
  ): ValidationResult {
  ...
  private static warnMedicalConditions(
    conditions: string[],
    aggressive: boolean,
  ): ValidationResult {
    const HIGH_RISK = [
  ...
  private static warnConcurrentTrainingInterference(
    goals: string[],
  ): ValidationResult {
  ...
  private static warnMedicationEffects(
    medications: string[],
  ): ValidationResult {
  ...
  private static calculateMacros(
    ...
    if (intensity === "advanced" && workoutFrequency >= 4) {
      carbPercent = 0.5; // 50% carbs for high volume training
    } else if (workoutFrequency >= 3) {
      carbPercent = 0.45; // 45% carbs for moderate training
    } else {
      carbPercent = 0.4; // 40% carbs for light training
    }
  ...
  private static applyMedicalAdjustments(
    ...
    if (
      ...
      adjustedTDEE = tdee * 0.9; // -10% for underactive thyroid
      ...
      notes.push(
        "💊 Monitor thyroid levels regularly - may change with treatment",
      );
      notes.push(
        "🩺 Consult doctor before starting - metabolism may be unstable",
      );
    }
    ...
    adjustedMacros.carbs = Math.max(adjustedMacros.carbs, macros.carbs * 0.7); // Never reduce more than 30%
  ...
  private static calculateRefeedSchedule(
    ...
    if (needsRefeeds) {
      ...
      explanation.push(
        "• Benefits: Prevents metabolic adaptation, restores leptin",
      );
    }
    ...
    if (needsDietBreak) {
      ...
      explanation.push(
        `• Week ${breakWeek}: Full week at maintenance calories`,
      );
  ...
  // ========================================================================
  // SMART RATE ALTERNATIVES
  // ========================================================================

  /**
   * Calculate smart rate alternatives based on user's metabolic data.
   * Returns a graduated spectrum of options from aggressive to comfortable,
   * plus exercise options that allow faster rates while eating at BMR.
   *
   * @param userRequestedRate - The weekly rate (kg/week) the user wants
   ...
   * @param gender - 'male' or 'female' for minimum calorie floor
   */
  static calculateSmartAlternatives(
    ...
    // Helper function to get description based on risk level
    ...
    // Process diet-only options
    for (const tier of rateTiers) {
      // Skip duplicates (e.g., if user's rate equals one of the tiers)
      if (
        !tier.isUserOriginal &&
        Math.abs(tier.rate - userRequestedRate) < 0.05
      ) {
        continue;
      }

      // Show all rate options - let user see faster AND slower alternatives
      // Previously this skipped rates > userRequestedRate, but that hides valid fast options
    ...
    // Process exercise options (eating at BMR + exercise burns extra)
    for (const exercise of exerciseOptions) {
      ...
      // Show all exercise options - let user see the impact of adding exercise

      // Skip if weekly rate is <= 0
      if (weeklyRate <= 0) {
        continue;
      }
      ...
      alternatives.push({
        ...
        weeklyRate: Math.round(weeklyRate * 100) / 100,
        dailyCalories: Math.round(bmr), // Eating at BMR
        bmrDifference: 0,
        timelineWeeks,
        riskLevel,
        icon: exercise.icon,
    ...
    // Sort: User original first, then diet options by rate descending, then exercise options
    alternatives.sort((a, b) => {
      if (a.isUserOriginal) return -1;
      if (b.isUserOriginal) return 1;

      // Diet options before exercise options
      if (!a.requiresExercise && b.requiresExercise) return -1;
      if (a.requiresExercise && !b.requiresExercise) return 1;

      // Within same category, sort by rate descending (fastest first)
      return b.weeklyRate - a.weeklyRate;
    });

    // Determine if we should show the rate comparison card
    // Show if there are multiple alternatives to choose from
    const userDailyDeficit = (userRequestedRate * CALORIE_PER_KG) / 7;
    const userDailyCalories = tdee - userDailyDeficit;
    const showRateComparison = alternatives.length > 1;
