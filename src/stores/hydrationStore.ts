  ...
  persist(
    (set, get) => ({
      ...
      addWater: (amountML: number) => {
        ...
        const maxIntake = state.dailyGoalML ? state.dailyGoalML * 1.5 : newIntake;
      ...
      checkAndResetIfNewDay: () => {
        ...
        if (state.lastResetDate !== today) {
          ...
          console.log('[HydrationStore] Daily reset triggered for new day:', today);
        }
      },
      ...
      getProgress: () => {
        ...
        return Math.min(100, Math.round((state.waterIntakeML / state.dailyGoalML) * 100));
      },
