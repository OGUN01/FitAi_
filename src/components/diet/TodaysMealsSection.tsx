  ...
  return (
    <Animated.View entering={FadeInRight.duration(400).delay(100 + index * 80)}>
      <AnimatedPressable
        ...
        hapticType="light"
      >
        <GlassCard elevation={1} blurIntensity="light" padding="md" borderRadius="lg" style={styles.mealCard}>
          <View style={styles.mealContent}>
            {/* Icon with gradient background */}
            <View style={[styles.mealIconContainer, { backgroundColor: `${config.color}15` }]}>
            ...
            <View style={styles.mealInfo}>
              <View style={styles.mealHeader}>
                ...
                <View style={[styles.timeBadge, { backgroundColor: `${config.color}15` }]}>
                  <Ionicons name="time-outline" size={rf(10)} color={config.color} />
                  <Text style={[styles.timeText, { color: config.color }]}>{config.time}</Text>
                </View>
              </View>
              ...
              <View style={styles.macroBadges}>
                <View style={[styles.macroBadge, { backgroundColor: 'rgba(255,107,107,0.12)' }]}>
                  <Text style={[styles.macroBadgeText, { color: '#FF6B6B' }]}>
                    P {Math.round(meal.protein)}g
                  </Text>
                </View>
                <View style={[styles.macroBadge, { backgroundColor: 'rgba(78,205,196,0.12)' }]}>
                  <Text style={[styles.macroBadgeText, { color: '#4ECDC4' }]}>
                    C {Math.round(meal.carbs)}g
                  </Text>
                </View>
                <View style={[styles.macroBadge, { backgroundColor: 'rgba(255,193,7,0.12)' }]}>
...
// Empty State
const EmptyMealsState: React.FC<{ onGenerate?: () => void }> = ({ onGenerate }) => (
  <Animated.View entering={FadeInRight.duration(400).delay(100)} style={styles.emptyStateWrapper}>
    <GlassCard elevation={1} blurIntensity="light" padding="lg" borderRadius="lg">
      <View style={styles.emptyState}>
        <View style={styles.emptyIconContainer}>
          <Ionicons name="restaurant-outline" size={rf(32)} color={ResponsiveTheme.colors.textSecondary} />
        </View>
        <Text style={styles.emptyTitle}>No meals planned</Text>
        <Text style={styles.emptySubtitle}>Generate a personalized meal plan to get started</Text>
...
export const TodaysMealsSection: React.FC<TodaysMealsSectionProps> = ({
  ...
  const completedCount = meals.filter(m => m.isCompleted || m.progress === 100).length;
    ...
    <View style={styles.container}>
      ...
      <View style={styles.sectionHeader}>
        <View style={styles.headerLeft}>
          <Ionicons name="calendar-outline" size={rf(18)} color={ResponsiveTheme.colors.primary} />
          <Text style={styles.sectionTitle}>Today's Meals</Text>
        </View>
        {meals.length > 0 && (
          <View style={styles.headerRight}>
            <View style={[styles.statusBadge, { backgroundColor: 'rgba(102,126,234,0.15)' }]}>
              <View style={[styles.statusDot, { backgroundColor: ResponsiveTheme.colors.primary }]} />
              <Text style={[styles.statusText, { color: ResponsiveTheme.colors.primary }]}>
