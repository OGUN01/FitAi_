...
const AchievementCelebration: React.FC<AchievementCelebrationProps> = ({
  ...
  // Confetti animations
  const confetti = useRef(Array.from({ length: 20 }, () => ({
  ...
  const getTierGradient = (tier: string) => {
    ...
    return gradients[tier as keyof typeof gradients] || 'from-orange-300 to-orange-600';
  };
        ...
        {confetti.map((item, index) => (
          <Animated.View
            ...
            style={[styles.confetti, {
              backgroundColor: index % 3 === 0 ? '#FFD700' : index % 3 === 1 ? '#FF6B6B' : '#4FC3F7',
              transform: [
                { translateX: item.x },
                { translateY: item.y },
                { scale: item.scale },
