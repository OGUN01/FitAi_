...
export function getRequiredNumericField(
  ...
  if (value === null || value === undefined) {
    throw new Error(`Required numeric field missing: ${fieldName}${contextMsg}`);
  }

  if (typeof value !== 'number' || isNaN(value)) {
    throw new Error(`Invalid numeric value for ${fieldName}: expected number, got ${typeof value}${contextMsg}`);
  }

  if (value < min || value > max) {
    throw new Error(`${fieldName} must be between ${min} and ${max}, got ${value}${contextMsg}`);
  }
...
export function getRequiredArrayField<T>(
  ...
  if (!Array.isArray(value)) {
    throw new Error(`Invalid array value for ${fieldName}: expected array, got ${typeof value}${contextMsg}`);
  }

  if (value.length < minLength) {
    throw new Error(`${fieldName} must have at least ${minLength} items, got ${value.length}${contextMsg}`);
  }
...
export function validatePersonalInfo(personalInfo: any): ValidationResult {
  ...
  const rules: FieldValidationRule[] = [
    ...
    {
      field: 'gender',
      validator: (v) => typeof v === 'string' && ['male', 'female', 'other'].includes(v.toLowerCase()),
...
export function validateBodyMetrics(bodyMetrics: any): ValidationResult {
  ...
  if (!bodyMetrics.activity_level) {
    warnings.push('Activity level not set - calorie calculations may be less accurate');
  }
...
export function validateDietPreferences(dietPrefs: any): ValidationResult {
  ...
  if (dietPrefs.dietary_restrictions && !Array.isArray(dietPrefs.dietary_restrictions)) {
    arrayValidations.push({
      isValid: false,
...
export function validateWorkoutPreferences(workoutPrefs: any): ValidationResult {
  if (!workoutPrefs) {
    return {
      isValid: false,
  ...
  const rules: FieldValidationRule[] = [
    {
      field: 'fitness_level',
      validator: (v) => typeof v === 'string' && ['beginner', 'intermediate', 'advanced'].includes(v.toLowerCase()),
  ...
  // Validate arrays if present
  if (workoutPrefs.preferred_workout_types && !Array.isArray(workoutPrefs.preferred_workout_types)) {
    return {
      isValid: false,
  ...
  if (workoutPrefs.available_equipment && !Array.isArray(workoutPrefs.available_equipment)) {
    return {
      isValid: false,
...
export function validateMinimumProfile(profile: any): ValidationResult {
  ...
  if (!profile.bodyMetrics?.current_weight_kg || profile.bodyMetrics.current_weight_kg < 30) {
