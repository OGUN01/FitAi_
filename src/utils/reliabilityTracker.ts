...
class ReliabilityTracker {
  ...
  generateReport(): ReliabilityReport {
    ...
    const completedIssues = this.issues.filter(
      (i) => i.status === "completed",
    ).length;
    const pendingIssues = this.issues.filter(
      (i) => i.status === "pending",
    ).length;
   ...
   */
  getIssuesBySeverity(
    severity: ReliabilityIssue["severity"],
  ): ReliabilityIssue[] {
  ...
  initializeWithKnownIssues(): void {
    ...
    this.addIssue({
      ...
      description:
        "String fontWeight values causing potential HostFunction errors",
      estimatedEffort: 2,
    });
    ...
    this.addIssue({
      ...
      file: "10+ files",
      description:
        "console.log statements need to be replaced with proper logging",
      estimatedEffort: 1,
    });
