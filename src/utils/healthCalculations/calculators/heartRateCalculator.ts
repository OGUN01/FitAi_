...
export class HeartRateCalculator {
  ...
  calculateZones(
    ...
    return {
      ...
      zone2: {
        name: "Aerobic",
        min: Math.round(resting + reserve * 0.6),
        max: Math.round(resting + reserve * 0.7),
        description: "Steady cardio, fat burn (60-70%)",
      },
      zone3: {
        name: "Tempo",
        min: Math.round(resting + reserve * 0.7),
        max: Math.round(resting + reserve * 0.8),
        description: "Tempo runs, moderate intensity (70-80%)",
      },
      zone4: {
        name: "Threshold",
        min: Math.round(resting + reserve * 0.8),
        max: Math.round(resting + reserve * 0.9),
        description: "Intervals, hard efforts (80-90%)",
      },
      zone5: {
        name: "VO2 Max",
        min: Math.round(resting + reserve * 0.9),
        max: max,
  ...
  calculateTargetHR(
    ...
    const rangeMin = Math.round(
      resting + reserve * ((intensityPercent - 5) / 100),
    );
