...
export class MuscleGainCalculator {
  ...
  calculateMaxGainRate(user: UserProfile): MuscleGainLimits {
    ...
    return {
      ...
      category,
      confidenceLevel: trainingYears > 0 ? 'high' : 'medium'
    };
  }
  ...
  validateGoal(
    ...
    if (targetGain <= maxGain * 1.3) {
      return {
        valid: true,
        severity: 'info',
        message: `Slightly optimistic (${requestedRate.toFixed(2)}kg/month). Natural limit is ~${limits.monthlyKg.toFixed(2)}kg/month, but perfect conditions might allow ${targetGain.toFixed(1)}kg in ${timelineMonths} months.`,
        achievementProbability: 50,
        suggestedTimeline: Math.ceil(targetGain / limits.monthlyKg),
        recommendations: [
          ...
          `Realistic timeline: ${Math.ceil(targetGain / limits.monthlyKg)} months`
        ]
      };
    }
    ...
    return {
      valid: true,
      severity: 'warning',
      message: `Very optimistic goal. Natural limit is ${maxGain.toFixed(1)}kg in ${timelineMonths} months (${limits.monthlyKg.toFixed(2)}kg/month for ${limits.category} lifters). Realistic expectation: ${maxGain.toFixed(1)}kg. Extra weight will likely be fat.`,
      ...
      recommendations: [
        ...
        `Consider realistic timeline: ${Math.ceil(targetGain / limits.monthlyKg)} months`,
  ...
  estimateFirstYearPotential(user: UserProfile): {
    ...
    const limits = this.calculateMaxGainRate(user);
    const baseYearly = limits.yearlyKg;
