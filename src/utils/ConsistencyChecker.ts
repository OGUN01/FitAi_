...
export class ConsistencyChecker {
  ...
  private validateLocalDataType(
    ...
    switch (dataType) {
      case "personalInfo":
        if (!data.first_name)
          errors.push("Missing required field 'first_name'");
  ...
  private generateRecommendations(): string[] {
    ...
    if (warnings.length > 0) {
      ...
      const hasWeightMismatch = warnings.some((e) =>
        ["current_weight_kg", "target_weight_kg", "height_cm"].includes(
          e.field,
        ),
      );
    ...
    if (this.discrepancies.length === 0) {
      recommendations.push(
        "All data is consistent between old and new systems.",
      );
    }
  ...
  getSummary(): { errors: number; warnings: number; info: number } {
    return {
      ...
      warnings: this.discrepancies.filter((d) => d.severity === "warning")
        .length,
