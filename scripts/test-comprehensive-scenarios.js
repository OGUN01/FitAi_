...
async function runTest(scenarioKey) {
  ...
  log(
    `   👤 ${profile.age}yo ${profile.gender}, ${profile.occupation_type}`,
    "cyan",
  );
  ...
  log(
    `   📅 ${profile.workoutsPerWeek}x/week, ${profile.workoutDuration} min/session`,
    "cyan",
  );
  log(
    `   🔧 Equipment: ${profile.availableEquipment.slice(0, 3).join(", ")}${profile.availableEquipment.length > 3 ? "..." : ""}`,
    "cyan",
  );
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/workout/generate`, {
      ...
      body: JSON.stringify({
        ...
        weeklyPlan: {
          activityLevel:
            profile.occupation_type === "desk_job" ? "sedentary" : "moderate",
          prefersVariety: true,
        },
      }),
    });
    ...
    if (expected.excluded) {
      const allExercises = (result.workouts || []).flatMap((w) =>
        (w.workout?.exercises || []).map((e) => e.name?.toLowerCase() || ""),
      );
      ...
      if (foundExcluded.length > 0) {
        log(
          `   ✗ Found excluded exercises: ${foundExcluded.join(", ")}`,
          "red",
        );
    ...
    if (expected.warnings) {
      ...
      if (foundWarnings.length > 0) {
        ...
        log(
          `   ⚠️  Expected warnings not found: ${expected.warnings.join(", ")}`,
          "yellow",
        );
      }
    }
    ...
    if (expected.medicalClearance) {
      const planDesc = result.planDescription || "";
      const hasClearance =
        planDesc.toLowerCase().includes("medical clearance") ||
    ...
    log(
      `\n${validationPassed ? "✅ Test PASSED" : "⚠️  Test PASSED with warnings"}`,
...
async function main() {
  ...
  log("═".repeat(100), "bright");
  log(
    `\n  Testing ${Object.keys(TEST_SCENARIOS).length} realistic scenarios from onboarding flow`,
    "cyan",
  );
  ...
  const passedWithWarnings = results.filter(
  ...
  if (successfulTests.length > 0) {
    ...
    log(
      `   Target: <1200ms ${maxDuration < 1200 ? "✓" : "✗"}`,
      maxDuration < 1200 ? "green" : "red",
    );
  }

  const successRate = (((passed + passedWithWarnings) / total) * 100).toFixed(
    1,
  );
  log(
